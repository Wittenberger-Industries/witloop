...
- Tests: `python -m unittest discover -s tests` → 286 OK (`.logs/gate-tests.txt`)
- Structure / CI-equivalent: `python scripts/validate.py` → exit 0 (`.logs/gate-ci.txt`)

### Safety fact
- Claim: the loaded checker charter, ship PR template, ship/RPA gate files, and 1.16.1 lockstep encode the locked Safety fact contract (always-on result-mode rows after the bug-fix table; `### Safety fact` between Testing and Verification; unproven does not skip gates; no D3 severity cluster).
- Proof: `python -m unittest tests.test_ship_safety_fact tests.test_work_type_release tests.test_work_type_docs tests.test_bug_fix_checker.CheckerBugFixMatrixTests tests.test_bug_fix_checker.CheckerPreservedContractTests tests.test_bug_fix_checker.ShipBugFixEvidenceTests tests.test_bug_fix_checker.ShipRulesInventoryTests` (this session; 67 tests, OK)

### Verification
Result-mode checker: `## CHECK PASSED`. All six ACs wired. No BLOCKER / WARNING / INFO findings. Line-level: ready to merge. Safety fact row was written while `PR.md` was absent (not a miss); this section is the copy.
