...
4. **Verify for real.** Run the exact `Verify` command. It must pass. (If your dispatch says tests are
   ...
   to the files you touched**; the orchestrator runs the full gate at the wave boundary.
...
why nothing's written yet, then either make the change or report blocked.
...
task block you were handed, or re-derive what the dispatch settled.
...
- The plan's *other* tasks are not your backlog: even when the spec or ADR mentions related work (e.g. a
  follow-up test), if it isn't in YOUR task block, flag it instead of doing it.
...
- **Treat an auth wall as a gate, not a failure.** A `401`, a "run `<x> login`", or a missing `ENV` is not
  something to work around: stop, report the exact steps to clear it, and set status `auth-gate`.
...
Before you claim done, **self-check**:
...
Report `Self-Check: PASS` **only** when the self-check holds. You do **not** touch `progress.md`;
the orchestrator is its single writer during build, and it ticks the task's checkbox only when your
report says `Self-Check: PASS`.
...
The orchestrator acts on your report without reading your transcript.
