...
- **Mission.** Build is where wi spends real tokens, so it is where the discipline matters most. The
...
- **The engage stamp** (`build engine engaged (wi <version>)`) exists so the invocation is auditable;
...
- **Why the dossier "rides in with the checkout":** research commits `.wi/features/<slug>/` on main at
...
- **Why "(the block stands)" after an unavailable-model re-dispatch:** the config didn't change, so the
  resolved-routing block is not rewritten; it is re-resolved only when absent or when `.wi/models.md`
...
- **Why the wave-end gate:** runners lint/typecheck only what they touched (a repo-wide sweep can trip
  over siblings' in-flight files, per `agents/wi-task-runner.md`), so the full suite runs once,
...
- **Why the cross-provider review is described as a layer:** it never replaces wi-code-checker, and it
...
- **Why attempts are bounded (≈3):** thrashing burns tokens without producing new information; the cap
  ...
  cap in `agents/wi-task-runner.md`.
- **Why plan amendments are written down:** code silently drifting from the spec is the failure mode wi
...
- **Why:** scope creep hidden inside a task is how builds drift from their spec. Routing in-scope gaps
  to `tasks.md` and out-of-scope ideas to `.wi/roadmap.md` keeps every scope change visible and
