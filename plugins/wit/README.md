...
type: Readme
title: "Witloop"
description: Spec-driven loop (setup, scan, dev, rpa, add-issues) for Claude Code, Copilot CLI, Grok Build, and Cursor. v1.16.5.
...
It runs on **four hosts**: Claude Code, GitHub Copilot CLI, Grok Build, and Cursor. Current release is **1.16.5**.
...
```
.
├── plugin.json          Copilot root manifest
...
The git repo around this directory is the **witloop** marketplace (`source`: `./plugins/wit`). Copilot copies this folder, not the repo root, so `.git` is not part of the install.

Before a release (from the marketplace repo root): `python plugins/wit/scripts/validate.py` and `python -m unittest discover -s tests`. Install PyYAML for a full frontmatter parse.
