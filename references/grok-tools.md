---
type: Reference
title: "Grok Build: tool & capability mapping for wit"
description: "Claude Code to Grok Build (grok CLI) tool-name and capability equivalents used when running wit on Grok, plus the mandatory resolve-once plugin-root protocol and the model-judged keep-alive pointer."
timestamp: 2026-07-12
tags: [grok, tools, portability, reference]
---

# Grok Build: tool & capability mapping for wit

wit's skills are written with Claude Code names. On Grok Build (`grok` CLI), use these equivalents.
...
2. **The flat `wit-*` aliases** into `~/.agents/skills/` (the shared flat-skills dir Grok scans; confirmed
   ...
   forms `/wit-scan`, `/wit-dev`, `/wit-rpa`. scan's bootstrap offers this copy
...
| wit/skill says | Grok equivalent |
|---|---|
...
| invoke a wit skill | user-invocable skills load as bare slash commands: `/scan`, `/dev`, `/rpa` (the phase skills are `user-invocable: false`, so `/ship` etc. are not commands). On a name clash the qualifier is **scope-based** (`/user:scan`, `/local:scan`), NOT `/wit:scan` (colon-qualification is agents-only), and a built-in of the same name wins. For a collision-free branded form, install the flat `wit-*` forwarding aliases into `~/.agents/skills/` (the shared flat-skills dir Grok scans, same target as Copilot/Codex; never into Grok's own `~/.grok/skills/`) -> `/wit-scan`, `/wit-dev`, `/wit-rpa`. Natural-language auto-trigger also works. (confirmed S4) |
...
Grok's tool is `spawn_subagent`. Dispatch each wit role as `general-purpose` with the role's contract
inlined into the prompt; do **not** assume a named wit agent resolves - registration is convenience, not
...
good: `cyan`, `green`, `red`, `blue`, `yellow`, `purple`, `orange`, `pink`). wit's charters use supported
colors; if an agent is missing from `grok inspect`, check `color` first.

| wit role | Grok dispatch |
|---------|----------------|
| `wit-task-runner` | `general-purpose`, prompt = inlined `agents/wit-task-runner.md` contract + task skeleton; **shared** feature worktree (`isolation: none`) |
| `wit-researcher` | `explore` or `general-purpose` + read-only; inline researcher contract |
| `wit-code-checker` | `general-purpose`; inline checker contract |
...
- **wit feature worktree** (`git worktree add -b wit/<slug> ...`) is canonical; the orchestrator is the sole
  committer.
- **Grok session `-w`** is an optional outer shell; do **not** run wit's worktree flow inside it. Measured
  ...
  **path**: cwd under `~/.grok/worktrees/` means you are in a session copy - follow the sandboxed variant
  (commit in place, no new worktrees, hand the user branch + commit + PR text; a `wit/<slug>` branch or
  ...
  wit repos have commits by build time).
