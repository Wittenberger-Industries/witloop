...
- `model_judged_goal`: Grok Build, Cursor
- `relaunch`: GitHub Copilot CLI
- `none`: no current host (chat persists; optional `/loop`; not an armed `/goal`)
...
Grok Build and Cursor share this family (native `/goal`, **model-judged**, not a predicate):

```
/goal The <slug> PR is open with its remote checks green (or none configured) and its branch passes <lint + test commands from repo-map.md>; .wit/features/<slug>/progress.md Phase is done. Constraints: only files named in tasks.md change; never force-push; tests are never weakened to pass.
```

The agent drives the goal itself and marks it complete when **it** judges the work done, so the
condition line is the **definition of done**, not a platform predicate the runtime enforces. Paste it
as **one line**. Grok registers/completes via `update_goal` (`grok-tools.md`). Cursor registers via
`CreateGoal` and completes via `UpdateGoal` (`cursor-tools.md`).
...
condition, not a hard predicate the way Claude/Codex `/goal` is. Keep the condition unambiguous.
Grok-only headless CLI (`grok -p`, `--always-approve`, `/goal pause | resume`) lives in
`grok-tools.md`, not in this shared print block.
...
No current host stamps `none`. Print this block when the stamped `keep_alive` cell is `none`.
Optional `/loop` is a user opt-in after Phase is done, not an armed predicate.
