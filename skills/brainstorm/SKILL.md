...
description: >
  ...
  this skill when the user types "/wit:brainstorm", as the interactive phase of "/wit:dev", or when they say
  "I have an idea", "help me think through what I want", or "let's scope a feature". This is the first of
  wit's two conversations: the technical "how" is designed autonomously and confirmed once at the design
  gate.
---

# brainstorm: pin down what the user wants

This is the **first** of two conversations in the wit loop (the second is the design gate). Capture *what* the feature should be (behavior,
...
the dialogue; wit's job is then only to capture the outcome into `.wit/features/<slug>/brief.md` in the
format below. Log the mode to `progress.md` either way (engine plus interactivity):
`brainstorm via superpowers:brainstorming` or `brainstorm via wit fallback (superpowers absent)`, suffixed
...
is installed is a defect, not a preference: wit exists to orchestrate the best installed tool, and the
...
**Scope of the delegation: wit supersedes the delegate's pipeline.** `superpowers:brainstorming` supplies
the *dialogue method*; its own artifact and handoff contract do not apply under wit. The outcome is
captured in wit's `brief.md` (no `docs/superpowers/specs/` design doc, and no separate spec-review gate
beyond the must-asks below), and its "the only skill you invoke next is writing-plans" ending is replaced
by wit's own pipeline (dev owns the handoff; research/plan fire in their phases: plan's delegation check
...
research*; don't invent it. A fully-specified idea text or a matching `.wit/roadmap.md` row **seeds** these
...
run without a user, fall back and stamp `via wit fallback, self-answered (headless)`. This is a fallback
...
Whether superpowers ran the dialogue or wit did, sharpen the language and persist it: vague terms are how
specs drift. Maintain project-level `.wit/glossary.md` (shared across features; create it lazily):
...
Glossary updates are committed where written (`docs(wit): glossary`, the project-level rule in
`wit-directory.md`).

## Fallback dialogue (ONLY when superpowers:brainstorming is absent)

1. **Load context.** Read `constitution.md` + `repo-map.md` (and `overview.md` + `.wit/glossary.md` if
