...
Design rationale for this skill lives in the wi repo's `docs/wi-design-notes/build.md` (maintainer doc,
...
skill instead. First act once engaged: append `build engine engaged (wi <version>)` to progress.md's
...
**The feature dossier rides in with the checkout.** Research committed `.wi/features/<slug>/` on main
...
2. **Dispatch the whole ready set at once**: one fresh `wi-task-runner` (`agents/wi-task-runner.md`) per
...
4. **As each report returns:** check its Verify result and **honor its `Self-Check` line**: tick
   ...
   first (python fallback: workflow.md "Script invocation"), per wi-directory.md's **ledger rule** (exact
...
output house rule (`… > .wi/features/<slug>/.logs/w<N>-tests.txt 2>&1`), verdict from the exit code +
...
cross-provider diff review** (the layer on top of wi-code-checker) over the wave's diff there
