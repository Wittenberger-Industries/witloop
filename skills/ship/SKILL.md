...
Design rationale for this skill lives in the wi repo's `docs/wi-design-notes/ship.md` (maintainer doc,
...
**Review dispatch (wi-code-checker · result mode): always runs; one dispatch, two passes.** First
...
template: none`. Log `review via wi-code-checker + superpowers:requesting-code-review[inline]` or
`review via wi-code-checker (wi line review; superpowers absent)`.

Then dispatch **wi-code-checker** (`agents/wi-code-checker.md`) in `result` mode, asking for both
...
wi-code-checker says isn't delivered. `cross-review.md` is ephemeral (pruned in ship:6, after ship:5
...
- **Architecture diagram:** a change that added or removed a module, dependency, layer, or external
  service updates `.wi/architecture.md` (mermaid graph + legend). Absent (a greenfield project's first
  feature) → create it from scan's template (scan's docs are committed where written,
  wi-directory.md's project-level rule, so absence really means greenfield). Then validate it:
  `python ${CLAUDE_PLUGIN_ROOT}/skills/scan/scripts/check_mermaid.py .wi/architecture.md`; fix every
  error it reports. (python fallback: workflow.md "Script invocation"; holds for every script in this
  SKILL.)
- **Project overview & commands:** organization/stack/run-step changes → `.wi/overview.md`; command
  changes → `.wi/repo-map.md`.
...
2. **The index `.wi/learnings.md`** (create lazily) is updated **every feature**: it is how later
...
- **Tidy the dossier** (BEFORE cutting the PR, so the PR carries a clean `.wi/`). Read the **`Flow:`**
  ...
  `dev` → `${CLAUDE_PLUGIN_ROOT}/skills/research/references/wi-directory.md`, `rpa` →
  ...
  1. *Sweep strays:* every feature-specific file lives under `.wi/features/<slug>/`: anything this run
     left loose in `.wi/` or elsewhere moves into the slug folder, or is deleted if worthless.
     Project-level files stay where they are: the whitelist is the flow reference's project-level list
     (dev: wi-directory.md's "Project-level memory" bullet; rpa: rpa-directory.md's "Project-level
  ...
  2. *Prune the ephemera*: exactly the flow reference's list (dev: wi-directory.md's ephemera bullet;
  ...
  3. *Finalize `tokens.md`: NOW, inside the dossier commit*, or it never rides the PR:
     ...
     parse fails (wi-directory.md's **ledger rule**: never a substitute, estimate, or fabricated
...
worktree and branch **mechanics stay wi's own (ship:8)**: never delegate the removal itself. (A
...
pruned it: `mkdir -p .wi/features/<slug>/.logs && printf '*\n' > .wi/features/<slug>/.logs/.gitignore`
...
redirected to `.wi/features/<slug>/.logs/pr-checks.txt`; the final table (its tail) is the evidence you
...
- [ ] `tokens.md` passes the structural gate: run
      `python ${CLAUDE_PLUGIN_ROOT}/skills/ship/scripts/check_tokens.py .wi/features/<slug>/tokens.md`;
...
- [ ] `.wi/learnings.md` index has this feature's line (and the detail file exists if one was
      warranted)
- [ ] dossier = exactly the flow's manifest (per progress.md `Flow:`, missing = dev; dev: the
      seven-file dossier in wi-directory.md; rpa: the run dossier in rpa-directory.md; **RPA runs: see
...
per-agent split when the finalize produced them. The two numbers wi trusts: **subagent-exact**
